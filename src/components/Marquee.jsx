export default function Marquee({ text, speed = 45, direction = 'left', className = '' }) {
  const chunk = Array(8).fill(`${text} · `).join('')
  return (
    <div className={`overflow-hidden border-y border-white/8 py-3 ${className}`}>
      <div
        className="inline-flex whitespace-nowrap"
        style={{ animation: `marquee ${speed}s linear infinite`, animationDirection: direction === 'right' ? 'reverse' : 'normal' }}
      >
